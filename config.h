...
static const Rule rules[] = {
    ...
    /** 普通优先度 */
    {"obs", NULL, NULL, 1 << 3, 0, 0, 0, -1, 0},      // obs        tag -> 
    ...
    keys[] =
        {
            ...
            {MODKEY | ShiftMask,
             ...
             tagmon,
             {.i =
                  +1}}, /* super shift b      |  将聚焦窗口移动到另一个显示器 */
            ...
            {MODKEY, XK_F1, spawn,
             SHCMD("light -U 5")}, /* super F1	    | 减小亮度 		     */
            {MODKEY, XK_F2, spawn,
             SHCMD("light -A 5")}, /* super F2	    | 增加亮度		     */
            {MODKEY, XK_s, togglescratch,
             SHCMD("tabbed -n scratchpad -c -r 2 st -w ''")}, /* super s |
                                                                 打开scratch终端
                                                               */
            {MODKEY, XK_Return, spawn,
             SHCMD("tabbed -n st -C tabbed -c -r 2 st -w ''")}, /* super enter |
                                                                   打开st终端 */
            {MODKEY, XK_minus, spawn,
             SHCMD("tabbed -n st -C FG -c -r 2 st -w ''")}, /* super + |
                                                               打开全局st终端 */
            {MODKEY, XK_space, spawn,
             SHCMD("tabbed -n st -C float -c -r 2 st -w ''")}, /* super space |
                                                                  打开浮动st终端
                                                                */
            {MODKEY, XK_F12, spawn,
             SHCMD("killall pcmanfm || pcmanfm")}, /* super F12        |
                                                      打开/关闭pcmanfm       */
            {MODKEY, XK_d, spawn,
             SHCMD("rofi -show run")}, /* super d          | rofi: 执行run */
            {MODKEY, XK_p, spawn,
             SHCMD("$DWM/DEF/rofi.sh")}, /* super p          | rofi:
                                            执行自定义脚本   */
            {MODKEY, XK_n, spawn,
             SHCMD("$DWM/DEF/blurlock.sh")}, /* super n          | 锁定屏幕 */
            {MODKEY, XK_F6, spawn,
             SHCMD("$DWM/DEF/set_vol.sh up")}, /* super F6   	    | 音量加 */
            {MODKEY, XK_F5, spawn,
             SHCMD("$DWM/DEF/set_vol.sh down")}, /* super F5         | 音量减 */
            ...
            TAGKEYS(XK_0, 6, "linuxqq") TAGKEYS(
                XK_w, 7, "/opt/apps/com.qq.weixin.deepin/files/run.sh")
                TAGKEYS(XK_y, 8,
